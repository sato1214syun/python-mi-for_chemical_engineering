...
threshold_of_r = 0.95  # 相関係数の絶対値がこの値以上となる特徴量の組は、同じクラスターになるように計算します  # noqa: E501
# 設定 ここまで
...
for i in range(num_of_clusters):
    ...
    if variable_numbers.shape[0] == 1:
        x_averaged = pd.concat(
            [x_averaged, x.iloc[:, variable_numbers[0]]], axis=1, sort=False
        )
    else:
        x_each_cluster = x.iloc[:, variable_numbers]
        autoscaled_x_each_cluster = (
            x_each_cluster - x_each_cluster.mean()
        ) / x_each_cluster.std()
        averaged_x_each_cluster = autoscaled_x_each_cluster.mean(axis=1)
        averaged_x_each_cluster.name = f"mean_in_cluster_{i}"
        x_averaged = pd.concat(
